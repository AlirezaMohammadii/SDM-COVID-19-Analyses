...
diseases = split(all_countries, f=all_countries2$Variable)

#sigma prime is standard daviation for each disease
a = data.frame()
for(i in 1:length(diseases)){
  d[i] <- data.frame(unique(diseases[[i]]$`standard daviation`)) 
  a <- d
}
#make matrix out of sigma prime list
mat_sigma_prime = matrix(unlist(a),nrow = 48, byrow = TRUE)
#transpose it to get precisly the matrix of what we had as a list
mat_sigma_prime = t(mat_sigma_prime)
write.csv(mat_sigma_prime,"C:\\Users\\mohammadia\\Desktop\\R\\Countries\\matrix_sigma_prime.csv", row.names = TRUE)
...
test_all_countries2 = test_results(diseases,all_countries)
...
concise_compelete = compelete[,c("Variable","pearson_test_pop","p_value_pop","coefficient_pop")]

#alpha double prime is coefficient_pop, we transpose it to be able to multiply by each row of sigma prime
alpha_double_prime = c(concise_compelete$coefficient_pop)

#to calculate alpha prime, we multiply sigma prime to alpha double prime, the formula below is used to 
#implement multiplication of a vector to each other rows of a matrix separately 
alpha_prime = t(t(mat_sigma_prime) * alpha_double_prime)
write.csv(alpha_prime,"C:\\Users\\mohammadia\\Desktop\\R\\Countries\\alpha_prime.csv", row.names = TRUE)

#upload population file
population = read.csv("population.csv")
#filter data by saving the last row 
avg_population = population[12,]
avg_population = (avg_population[,-c(1)])

#to calculate alpha for each country which intuitively is the number of mortality cases caused by COVID-19,
#we must multiply alpha prime by the average of population for each country.
alpha = ""
for(i in 1:48){
  for (j in 1:14){
    d[i,j] = data.frame(avg_population[j] * alpha_prime[j,i])
    alpha <- d
  }
}
#alpha = (matrix(unlist(alpha),ncol = 14, nrow = 48))
#drop decimal part of each cell

#adding morbidity names to the first column of data
disease_names = unlist(unique(all_countries$Variable))
alpha = cbind(disease_names, alpha)
...
write.csv(alpha,"C:\\Users\\mohammadia\\Desktop\\R\\Countries\\alpha.csv", row.names = TRUE)
